    ...
    def get_last_successful_run(self):
        try:
            ...
            if end_time:
                ...
                return f"{dt.strftime('%d %b %Y  %I:%M %p')}  (by {user})"
        except Exception as e:
...
class ReceiptFormatter:
    ...
    def format_bill(self, bill, items_data=None, extra_info=None):
        ...
        if combined_gst > 0:
            pre_tax = round(bill['subtotal'] / (1 + combined_gst / 100), 2)
        else:
            pre_tax = bill['subtotal']
...
class RestaurantApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(nav, text="RESTO BILL", font=("Georgia", 28, "bold"),
                 fg="#FFA500", bg="#0a0a0a").pack(side="left", padx=20)
    ...
    def new_order(self):
        ...
        # No active order — nothing to save, screen is already fresh
        messagebox.showinfo("New Order", "Screen is already ready for a new order.")
    ...
    def add_selected_item(self):
        ...
        if not self.active_order_id:
            ...
            if btype == "Dine-In":
                ...
                # Ask customer details BEFORE creating the order
                item_to_add = item
                def _after_del(n, p, a):
                    tbl   = f"DEL-{uuid.uuid4().hex[:4].upper()}"
                    notes = f"Phone: {p} | Address: {a}"
                    oid, onum = self.db.create_order(tbl, "delivery",
                                                     customer_name=n, notes=notes)
                    ...
                    self._update_status()
                    self.refresh_order_display()
                    self._qty_popup(item_to_add["name"], item_to_add["price"],
                                    item_to_add["category"])
                self._delivery_customer_popup(_after_del)
                return
        ...
        if not self.active_order_id:
            ...
            if btype == "Dine-In":
                ...
                def _after_del2(n, p, a):
                    ...
                    self._update_status()
                    self.refresh_order_display()
                    self._add_extra_item_popup()
                self._delivery_customer_popup(_after_del2)
                return
        curr = self.db.get_setting("currency", "Rs.")
    ...
    def _finalise_bill(self):
        ...
        self.active_order_id = self.active_order_number = self.active_table = None
        self.table_no_var.set(""); self.discount_var.set("0")
        self._order_item_ids = []
        self.cart_listbox.delete(0, "end")
        curr = self.db.get_setting("currency", "Rs.")
        self.total_lbl.config(text=f"Total: {curr} 0.00")
